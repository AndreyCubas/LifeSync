...
export function FitnessModule() {
  ...
  return (
    <div className="p-8 max-w-5xl">
      <PageHeader
        title="Fitness & Saúde"
        subtitle="Registo de refeições, treinos e evolução de cargas"
      />
...
function NutritionTab() {
  ...
  const f = <K extends keyof MealFormData>(k: K, v: MealFormData[K]) =>
    setForm((p) => ({ ...p, [k]: v }));
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-5">
        <div className="flex items-center gap-2">
          <Button
            onClick={() => shiftDate(-1)}
            className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors"
          >
            <ChevronLIcon />
          </Button>
          <Input
          ...
          <Button
            ...
            className="text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950 px-3 py-2 rounded-xl hover:bg-indigo-100 dark:hover:bg-indigo-900 border border-indigo-200 dark:border-indigo-800 transition-colors"
          >
            Hoje
          </Button>
        </div>
      ...
      <div className="grid grid-cols-5 gap-3 mb-6">
        {[
          ...
          },
          {
            ...
            unit: `/${DEFAULT_MACRO_GOALS.protein}g`,
            color: "text-indigo-600 dark:text-indigo-400",
          },
          {
            label: "Carbos",
            value: `${totals.carbs}`,
            unit: `/${DEFAULT_MACRO_GOALS.carbs}g`,
            color: "text-amber-600 dark:text-amber-400",
          },
          {
            label: "Gorduras",
            value: `${totals.fats}`,
            unit: `/${DEFAULT_MACRO_GOALS.fats}g`,
            color: "text-pink-600 dark:text-pink-400",
          },
          {
            label: "Fibras",
            value: `${totals.fiber}`,
            unit: `/${DEFAULT_MACRO_GOALS.fiber}g`,
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState
            icon={<AppleIcon />}
            text="Nenhuma refeição registada hoje."
            action={
      ...
      <Modal
        ...
        title={editingId ? "Editar refeição" : "Registar refeição"}
      >
        <div className="grid grid-cols-2 gap-4">
          ...
          <FormField label="Proteínas (g)">
            <Input
              type="number"
              value={form.protein as string}
              onChange={(e) => f("protein", e.target.value)}
              placeholder="0"
            />
          </FormField>
          <FormField label="Carboidratos (g)">
            <Input
              type="number"
              value={form.carbs as string}
              onChange={(e) => f("carbs", e.target.value)}
              placeholder="0"
            />
          </FormField>
          <FormField label="Gorduras (g)">
            <Input
              type="number"
              value={form.fats as string}
              onChange={(e) => f("fats", e.target.value)}
              placeholder="0"
            />
          </FormField>
          <FormField label="Fibras (g)">
            <Input
              type="number"
              value={form.fiber as string}
              onChange={(e) => f("fiber", e.target.value)}
              placeholder="0"
            />
          </FormField>
          <div className="col-span-2">
...
function MealCard({
  ...
  return (
    ...
    >
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl shrink-0 transition-transform duration-200 ${hov ? "scale-110" : ""}`}
      >
        🍽️
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {isNew && (
            <span className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950 px-2 py-0.5 rounded-full animate-pulse">
              ✓ registado
            </span>
          )}
        </div>
      ...
      <div
        className={`flex gap-2 transition-opacity duration-150 ${hov ? "opacity-100" : "opacity-0"}`}
      >
        <Button
          variant="outline"
          size="sm"
...
function WorkoutsTab() {
  ...
  const { exercises, refresh: refreshEx } = useExercisesForWorkout(
  ...
  const openCreateW = () => {
    setEditWId(null);
    setWForm({ ...EMPTY_WORKOUT, date: todayISO() });
    setWModal(true);
  };
  const openEditW = (w: Workout) => {
    setEditWId(w.id);
    setWForm({ name: w.name, date: w.date });
    setWModal(true);
  };
  const openCreateE = () => {
    setEditEId(null);
    setEForm({ ...EMPTY_EXERCISE });
    setEModal(true);
  };
  ...
  const wf = <K extends keyof WorkoutFormData>(k: K, v: WorkoutFormData[K]) =>
    setWForm((p) => ({ ...p, [k]: v }));
  const ef = <K extends keyof ExerciseFormData>(k: K, v: ExerciseFormData[K]) =>
    setEForm((p) => ({ ...p, [k]: v }));
  ...
  return (
    <div className="grid grid-cols-3 gap-5">
      ...
      <div>
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            text="Nenhuma ficha"
            action={
              <Button size="sm" icon={<PlusIcon />} onClick={openCreateW}>
                Criar ficha
              </Button>
            }
          />
          ...
          <div className="flex flex-col gap-2">
            {workouts.map((w) => (
              ...
              >
                <p
                  className={`text-sm font-semibold ${selectedWorkout?.id === w.id ? "text-indigo-700 dark:text-indigo-300" : "text-slate-900 dark:text-slate-100"}`}
                >
                  {w.name}
                </p>
      ...
      <div className="col-span-2">
        {!selectedWorkout ? (
          <Card>
            <EmptyState
              icon={<DumbbellIcon />}
              text="Seleciona ou cria uma ficha de treino"
            />
          </Card>
        ) : (
          <Card padding={false}>
            <div className="flex justify-between items-center p-5 border-b border-slate-100 dark:border-slate-700">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  size="sm"
                  icon={<EditIcon />}
                  onClick={() => openEditW(selectedWorkout)}
                >
                  Editar
                </Button>
                <Button
                  ...
                  size="sm"
                  icon={<TrashIcon />}
                  onClick={() => setConfirmWId(selectedWorkout.id)}
                >
                  Excluir
                </Button>
                <Button size="sm" icon={<PlusIcon />} onClick={openCreateE}>
                  Exercício
                </Button>
              </div>
            </div>
                ...
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-700">
                    {["Exercício", "Séries", "Reps", "Carga", ""].map((h) => (
                      <th
                        key={h}
                        className="text-left px-4 py-3 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest"
                      >
                        {h}
                      </th>
                    ))}
      ...
      <Modal
        ...
        title={editWId ? "Editar ficha" : "Nova ficha de treino"}
        width="max-w-sm"
      >
        <FormField label="Nome">
          <Input
            value={wForm.name}
            onChange={(e) => wf("name", e.target.value)}
        ...
        <FormField label="Data">
          <Input
            type="date"
            value={wForm.date}
            onChange={(e) => wf("date", e.target.value)}
          />
        </FormField>
        <div className="flex gap-3 justify-end">
          <Button variant="ghost" onClick={() => setWModal(false)}>
            Cancelar
          </Button>
          <Button loading={saving} onClick={saveWorkout}>
            {editWId ? "Salvar" : "Criar"}
          </Button>
        </div>
      </Modal>
      ...
      <Modal
        ...
        title={editEId ? "Editar exercício" : "Adicionar exercício"}
      >
        <FormField label="Exercício">
          <Autocomplete
            value={eForm.exercise_name as string}
            onChange={(v) => ef("exercise_name", v)}
            suggestions={EXERCISE_LIST}
        ...
        <div className="grid grid-cols-3 gap-4">
          <FormField label="Séries">
            <Input
              type="number"
              value={eForm.sets as string}
              onChange={(e) => ef("sets", e.target.value)}
              placeholder="4"
            />
          </FormField>
          <FormField label="Reps">
            <Input
              type="number"
              value={eForm.reps as string}
              onChange={(e) => ef("reps", e.target.value)}
              placeholder="10"
            />
          </FormField>
          <FormField label="Carga (kg)">
            <Input
              type="number"
              value={eForm.weight as string}
              onChange={(e) => ef("weight", e.target.value)}
              placeholder="60"
            />
          </FormField>
        </div>
        <div className="flex gap-3 justify-end">
          <Button variant="ghost" onClick={() => setEModal(false)}>
            Cancelar
          </Button>
          <Button loading={saving} onClick={saveExercise}>
            {editEId ? "Salvar" : "Adicionar"}
          </Button>
        </div>
      </Modal>

      <ConfirmDialog
        open={Boolean(confirmWId)}
        title="Excluir ficha"
        message="Isto irá remover a ficha e todos os exercícios. Continuar?"
        onConfirm={deleteWorkout}
        onCancel={() => setConfirmWId(null)}
      />
      <ConfirmDialog
        open={Boolean(confirmEId)}
        title="Excluir exercício"
        message="Remover este exercício?"
        onConfirm={deleteExercise}
        onCancel={() => setConfirmEId(null)}
      />
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-3">
        <div
          className={`flex gap-2 transition-opacity ${hov ? "opacity-100" : "opacity-0"}`}
        >
          <Button
            variant="outline"
            size="sm"
...
function ProgressTab() {
  ...
  if (history.length === 0) {
    return (
      <EmptyState
        icon={<TrendIcon />}
        text="Regista pelo menos 2 treinos com os mesmos exercícios para ver a evolução"
      />
    );
  }
        ...
        <Card key={name}>
          ...
          <div className="flex gap-6 mb-4">
            ...
            <div>
              ...
              <p className="text-2xl font-extrabold text-slate-400 dark:text-slate-500">
                {prev.weight}kg
              </p>
              <p className="text-xs text-slate-400 dark:text-slate-500">
                {prev.sets}×{prev.reps}
            ...
            {sorted.map((entry, i) => {
              ...
              return (
                <div
                  ...
                  style={{
                    height: `${(entry.weight / max) * 100}%`,
                    background: isLast ? "#6366f1" : "#e0e7ff",
                  }}
                />
              );
            })}
